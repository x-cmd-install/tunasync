# tunasync

[English version](./README.md)

Mirror job management tool. 

[![x-cmd/install — tunasync Code Quality Monitoring Repo Card](https://x-cmd.com/repo-card/tunasync.svg?lang=zh)](https://x-cmd.com/install/tunasync)

## 安装

```sh
x install tunasync
```

## 代码洞察

合计: **152,984** 行代码（覆盖前 5 种语言、共 **70** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| Json | 142,864 | 0 | 0 | 2 |
| Go | 9,309 | 357 | 1,363 | 61 |
| Bitbake | 760 | 15 | 120 | 5 |
| Makefile | 20 | 1 | 8 | 1 |
| Zsh | 12 | 0 | 3 | 1 |

## OpenSSF Scorecard 评分

总评分: **3.6 / 10**

评分最低的几项:

- **Maintained** (0/10) — 0 commit(s) and 0 issue activity found in the last 90 days -- score normalized to 0
- **Code-Review** (3/10) — Found 3/10 approved changesets -- score normalized to 3
- **Token-Permissions** (0/10) — detected GitHub workflow tokens with excessive permissions

## 源代码

- **上游仓库**: <https://github.com/tuna/tunasync>
- **许可证**: GPL-3.0

## 发布

- **最新版本**: `v0.10.0` (2026-04-13)
- **最近提交**: 2026-05-15
- **Release 含资产**: 4 个

## 流行度

- **Star**: 1,869 · **Fork**: 305 · **开放 issue**: 128 · **贡献者**: 29

## 累计统计

- **发布数**: 30 · **已合并 PR**: 65 · **开放 PR**: 5 · **已关闭 issue**: 119 · **开放 issue**: 9 · **提交数**: 536

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-27 | 0 | 0 | 1 | 0 | 0 | 0 |
| last60d | 2026-07-28 | 0 | 0 | 2 | 0 | 0 | 0 |
| 90d | 2026-06-28 | 0 | 0 | 4 | 0 | 0 | 0 |
| last180d | 2026-03-30 | 1 | 3 | 5 | 0 | 0 | 17 |
| 360d | 2025-10-01 | 1 | 3 | 5 | 0 | 0 | 17 |
| last720d | 2024-10-06 | 5 | 7 | 5 | 9 | 0 | 57 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [tunasync-v0.10.0-linux-amd64-bin.tar.gz](https://github.com/tuna/tunasync/releases/download/v0.10.0/tunasync-v0.10.0-linux-amd64-bin.tar.gz) | 29.8 MiB | `native/linux/x64` |
| [tunasync-v0.10.0-linux-arm64-bin.tar.gz](https://github.com/tuna/tunasync/releases/download/v0.10.0/tunasync-v0.10.0-linux-arm64-bin.tar.gz) | 27.2 MiB | `native/linux/arm64` |
| [tunasync-v0.10.0-linux-loong64-bin.tar.gz](https://github.com/tuna/tunasync/releases/download/v0.10.0/tunasync-v0.10.0-linux-loong64-bin.tar.gz) | 28.2 MiB | `native/unknown` |
| [tunasync-v0.10.0-linux-riscv64-bin.tar.gz](https://github.com/tuna/tunasync/releases/download/v0.10.0/tunasync-v0.10.0-linux-riscv64-bin.tar.gz) | 28.4 MiB | `native/linux/riscv64` |

## 改进这些数据

tunasync 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `tunasync` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/tunasync.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260926.yml` · 2026-09-26T05:23:51Z._
